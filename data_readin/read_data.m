...
for sub=4%:length(allsubs)
    ...
    sel_folder=strcat(path_data,sel_sub,'\ieeg\');
    cd (sel_folder)
    if strcmp(sel_sub,'c_sub21')
        filename=dir('**\*.eeg');
        cfg=[];
        cfg.dataset=fullfile(filename.folder,filename.name);
        ...
        filename=dir('*.edf');
        % new files are mostly cut in two sessions, append both session into one
        if numel(filename)>1
            for f=1:numel(filename)
                cfg            = [];
                cfg.dataset    = filename(f).name;
                cfg.continuous = 'yes';
                cfg.channel    = 'all';
                data{f}          = ft_preprocessing(cfg)
            end
            
            cfg=[];
            data_all_tmp=ft_appenddata(cfg,data{:})
            
            data_all.trial{:}=[data_all_tmp.trial{:}];
            data_all.sampleinfo=[1,size(data_all.trial{1},2)];
            data_all.fsample=data{1}.fsample;
            data_all.time{1}=0:1/data_all.fsample:(1/data_all.fsample*(data_all.sampleinfo(2)-1));
            data_all.label=data_all_tmp.label;
            data_all.cfg=data_all_tmp.cfg;
            clear data data_all_tmp
        else
    ...
    if strcmp(sel_sub, 'c_sub03')|| strcmp(sel_sub, 'c_sub04') || strcmp(sel_sub, 'c_sub05') || strcmp(sel_sub, 'c_sub06')  || strcmp(sel_sub, 'c_sub08')...
            || strcmp(sel_sub, 'c_sub11') || strcmp(sel_sub, 'c_sub12')|| strcmp(sel_sub, 'c_sub14') || strcmp(sel_sub, 'c_sub15') || strcmp(sel_sub, 'c_sub18')|| strcmp(sel_sub, 'c_sub22')|| strcmp(sel_sub, 'c_sub29')
    ...
    for i=1:numel(data_all.label)
        if ~isempty(strfind(data_all.label{i},'POL'))
            data_all.label{i}=data_all.label{i}(5:end);
            if isempty(data_all.label{i})
                data_all.label{i}='X';
            end
        end
        if ~isempty(strfind(data_all.label{i},'EEG'))
            data_all.label{i}=data_all.label{i}(5:end);
            if isempty(data_all.label{i})
                data_all.label{i}='X';
            end
    ...
    elseif strcmp(sel_sub,'c_sub19')
        clean_chan=data_all.label([1:19,21:31,33:37,54:55,58:157,164]);
    elseif strcmp(sel_sub,'c_sub21')
        clean_chan=data_all.label(setdiff(1:numel(data_all.label),[20, 30:33,38:49,52:53,146:151,156:158]));
    ...
    elseif strcmp(sel_sub,'c_sub25')
        clean_chan=data_all.label(setdiff(1:numel(data_all.label),[20,38:41,44:45,156:190]));
    elseif strcmp(sel_sub,'c_sub26')
        clean_chan=data_all.label(setdiff(1:numel(data_all.label),[20,38:41,44:45,126:130]));
        
    elseif strcmp(sel_sub,'c_sub29')
         clean_chan=data_all.label(setdiff(1:numel(data_all.label),[20,38:53,56:57,158:165]));
    else
    ...
    if strcmp(sel_sub,'c_sub09')
        diff_tresh=4.5.*data.fsample; % 4.5 sec min diff
    else
