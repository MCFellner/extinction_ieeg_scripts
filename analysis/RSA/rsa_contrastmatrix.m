...
%                 nan_mat(:,nan_col)=nan;
            ...
            case {'block1',  'block2', 'block3'}
             ...
             case {'cond1',  'cond2', 'cond3'}
                def_vec=trlinfo(:,6)==str2double(sel_contrast(end));
                contrast_mat=def_vec*def_vec';
                contrast_mat(contrast_mat==0)=NaN;
                clear def vec 
            ...
            case 'no_ustrials'    
             ...
             case 'video_specific'    
                sel_col=trlinfo(:,3);
                all_vid=unique(sel_col);
                for i=1:numel(all_vid)
                    tmp=sel_col==all_vid(i);
                    tmp_mat=tmp*tmp';
                    contrast_mat=contrast_mat+tmp_mat;
                end
                clear sel_col all_vid tmp tmp_mat
            ...
            case 'trial_slidingavg_def'
                ...
                % mask trialslide to exclude correlations across blocks
                def_vec=trlinfo(:,2)==1;
                mask_mat1=def_vec*def_vec';
                def_vec=trlinfo(:,2)==2;
                mask_mat2=def_vec*def_vec';
                   def_vec=trlinfo(:,2)==3;
                mask_mat3=def_vec*def_vec';
                mask_mat= mask_mat1+ mask_mat2+ mask_mat3;
                
                contrast_mat3d=contrast_mat3d.*repmat(reshape(mask_mat,[1,size(mask_mat)]),max(trlinfo(:,17)),1,1);
                contrast_mat=squeeze(nansum(contrast_mat3d,1));
                clear mask_mat1 mask_mat2 mask_mat3 def_vec min_rep max_rep
        end
